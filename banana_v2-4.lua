...
-- ==========================================
Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Nopermc21/test/refs/heads/main/ui_BananaHub_pink_blue.lua"))()
...
-- ===============================================================
-- ========== HOP SERVER SỰ KIỆN ==========
-- Logic gốc từ hop.txt (araujozwx)
-- API: http://n1.hanamc.io.vn:25568/qvuong/api/{name}
-- ===============================================================
do
    -- Dùng đúng theo logic gốc hop.txt
    local _reqfunc = request or (http and http.request) or http_reqest
    local _httpser = game:GetService("HttpService")
    local _tpser   = game:GetService("TeleportService")
    local _lp      = game:GetService("Players").LocalPlayer

    local function _getjob(name)
        local jobs = {}
        name = tostring(name or "fullmoon")
        local bool, tbl = pcall(function()
            return _reqfunc({
                Url    = "http://n1.hanamc.io.vn:25568/qvuong/api/" .. name,
                Method = "GET"
            })
        end)
        if not (bool and tbl) then
            print("[HDanh Hub] failed to send request: " .. name)
            return jobs
        end
        local decodedbody = _httpser:JSONDecode(tbl.Body)
        for i, v in pairs(decodedbody) do
    ...
    local function _tp(place, job)
        return _tpser:TeleportToPlaceInstance(tonumber(place), tostring(job), _lp)
    end

    local function _HopEvent(str)
        task.spawn(function()
            if not _reqfunc then
                print("[HDanh Hub] no req func")
                return
            end
            local jobs = _getjob(str)
            if #jobs <= 1 then
                print("[HDanh Hub] min 1, failed to fetch at least 1 jobid for: " .. str)
                return
            end
            for i, v in ipairs(jobs) do
                if v and (v.Job_id and v.place_id) then
                    local place, job = v.place_id, v.Job_id
                    if tonumber(game.PlaceId) == tonumber(v.place_id) then
                        warn("[HDanh Hub] hopping to: " .. str)
                        _tp(place, job)
                        task.wait(1)
                    end
                end
    ...
    -- ========== SỰ KIỆN THƯỜNG ==========
    Tabs.Hop:AddSection("Sự Kiện Thường")

    Tabs.Hop:AddButton({
        ["Title"] = "🌕 Hop Full Moon",
        ["Callback"] = function() _HopEvent("fullmoon") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🏝️ Hop Mirage Island",
        ["Callback"] = function() _HopEvent("mirage") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🦕 Hop Prehistoric",
        ["Callback"] = function() _HopEvent("prehistoric") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "👑 Hop Dough King",
        ["Callback"] = function() _HopEvent("doughking") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🐉 Hop Rip Indra",
        ["Callback"] = function() _HopEvent("ripindra") end
    })

    -- ========== HOP HAKI ==========
    Tabs.Hop:AddSection("Hop Haki Legendary")

    Tabs.Hop:AddButton({
        ["Title"] = "⚡ Hop Haki Legendary (chung)",
        ["Callback"] = function() _HopEvent("haki") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚡ Hop Haki Sworn With E",
        ["Callback"] = function() _HopEvent("hakiswonwithe") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "❄️ Hop Haki Winter Sky",
        ["Callback"] = function() _HopEvent("hakiwintersky") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🔴 Hop Haki Pure Red",
        ["Callback"] = function() _HopEvent("hakipurered") end
    })

    -- ========== HOP SWORD ==========
    Tabs.Hop:AddSection("Hop Sword Legendary")

    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Legendary (chung)",
        ["Callback"] = function() _HopEvent("sword") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Saishi",
        ["Callback"] = function() _HopEvent("swordsaishi") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Oroshi",
        ["Callback"] = function() _HopEvent("swordoroshi") end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Shizu",
        ["Callback"] = function() _HopEvent("swordshizu") end
    })
